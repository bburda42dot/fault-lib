...
fn multi_catalog_fault_isolation() {
    ...
    assert_eq!(cabin_temp.typed_status.as_ref().unwrap().confirmed_dtc, Some(true));
    ...
    assert_eq!(display.typed_status.as_ref().unwrap().confirmed_dtc, Some(true));

    // Cross-check: HVAC blower fault should NOT be affected by IVI.
    let blower = hvac_faults.iter().find(|f| f.code == "hvac.blower.speed_sensor_mismatch").unwrap();
    assert_eq!(
...
fn same_fault_id_different_catalogs_independent() {
    ...
    let record = make_fault_record(FaultId::Text(to_static_short_string("comm_error").unwrap()), LifecycleStage::Failed);
    ...
    assert_eq!(faults_a[0].typed_status.as_ref().unwrap().confirmed_dtc, Some(true));
...
fn delete_faults_scoped_to_catalog() {
    ...
    harness
        .processor
        .process_record(&hvac_path, &make_fault_record(FaultId::Numeric(0x7001), LifecycleStage::Failed));
    ...
    let hvac_faults = harness.manager.get_all_faults("hvac").unwrap();
    let cabin_temp = hvac_faults.iter().find(|f| f.code == "0x7001").unwrap();
    assert_eq!(cabin_temp.typed_status.as_ref().unwrap().confirmed_dtc, Some(false));
