...
fn json_string_catalog_e2e() {
    ...
    assert_eq!(fault_a.typed_status.as_ref().unwrap().test_failed, Some(true));
    assert_eq!(fault_a.fault_name, "JsonFaultA");
    assert_eq!(fault_a.symptom.as_deref(), Some("First fault loaded from JSON"));
}
...
fn json_file_catalog_e2e() {
    ...
    assert!(desc.compliance.iter().any(|c| matches!(c, ComplianceTag::EmissionRelevant)));
}
...
fn json_string_with_debounce_and_reset() {
    ...
    let catalog = FaultCatalogBuilder::new().json_string(json).unwrap().build();
