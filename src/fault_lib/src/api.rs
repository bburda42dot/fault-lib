...
use crate::enabling_condition::{EnablingCondition, EnablingConditionCallback, EnablingConditionError, EnablingConditionManager, FaultMonitor};
...
impl FaultApi {
    ...
    pub fn try_new(catalog: FaultCatalog) -> Result<FaultApi, InitError> {
        ...
        if !concrete_sink.check_catalog_hash(&catalog)? {
            return Err(InitError::CatalogVerification(SinkError::Other("catalog hash mismatch with DFM".into())));
        }
    ...
    pub(crate) fn try_get_fault_sink() -> Result<Arc<dyn FaultSinkApi>, SinkError> {
        STATE
            .get()
            .ok_or(SinkError::Other(alloc::borrow::Cow::Borrowed("FaultApi not initialized")))?
            .sink
            .upgrade()
            .ok_or(SinkError::Other(alloc::borrow::Cow::Borrowed("FaultApi has been dropped")))
    }
    ...
    pub fn get_enabling_condition(entity: &str) -> Result<EnablingCondition, EnablingConditionError> {
    ...
    ///
    /// - `EnablingConditionError::NotInitialized` if FaultApi was not initialized.
    pub fn create_fault_monitor(condition_ids: &[&str], callback: impl EnablingConditionCallback) -> Result<FaultMonitor, EnablingConditionError> {
        let manager = Self::try_get_ec_manager().ok_or(EnablingConditionError::NotInitialized)?;
...
mod tests {
    ...
    fn get_enabling_condition_before_init_returns_error() {
        ...
        assert!(matches!(result, Err(EnablingConditionError::NotInitialized)));
    }
